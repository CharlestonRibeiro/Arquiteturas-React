# Estudos de Arquitetura (React)

```
Arquitetura são as decisões significativas sobre a organização de um sistema de software.
...
Este repositório é o espelho web do [Arquiteturas-Flutter](../Arquiteturas-Flutter): mesmas fronteiras de camada e mesma direção de dependência, com implementação idiomática do ecossistema React — componentes de função + hooks, `Zustand` para estado, `TanStack Query` para data-fetching/cache.

## Decisões Básicas

Aqui estão os conceitos básicos de programação que são decisivos em todos os projetos:

- [Classes](basic/classes.md), [Métodos](basic/methods.md), [Variáveis](basic/variables.md), [Comentário ](basic/comments.md), [Imports](basic/imports.md), [Pacotes](basic/packages.md), [SOLID](basic/solid.md), [Testes](basic/tests.md), [GitHub](gitHub/gitHub.md).
...
### [MVC+R](architecture/mvc_r/MVC+R.md)
...
>
> **O**: Indica que a arquitetura também é aceita para o tipo de projeto. Embora não seja a opção mais indicada, ainda é uma escolha viável, mas exige mais disciplina manual do time para compensar o que a arquitetura recomendada (X) já estrutura de fábrica.

A recomendação é baseada em duas dimensões técnicas do projeto:
- **Fontes de dado:** quantas fontes externas o app consome (API, storage local) e se são estáveis ou sujeitas a troca/migração futura.
- **Complexidade de domínio:** quantos domínios de negócio/usecases desacoplados o app tem, e se há exigência de testabilidade rigorosa e vida útil longa.

| Perfil do projeto | Clean Modular | Flex Domain | MVC+R |
|:----------------|:-------------:|:-----------:|:-----:|
| Uma única fonte de dado (uma API **ou** um storage local), estável, domínio simples |               |             |   X   |
| Múltiplas fontes de dado, ou fonte sujeita a troca/migração futura |               |      X      |       |
| Múltiplas fontes de dado **e** múltiplos domínios de negócio / muitos usecases desacoplados |       X       |      O      |       |
| Domínio único, mas com exigência de testabilidade rigorosa e vida útil longa do projeto |       X       |      O      |       |

> [!IMPORTANT]
> Se o projeto se enquadra em mais de uma linha, use a **mais abaixo na tabela** (a de maior complexidade prevalece).
